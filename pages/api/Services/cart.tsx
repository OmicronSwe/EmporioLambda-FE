...
export const createEmptyCart = async (params): Promise<boolean> => {
    ...
    const {response} = (await getlambdaResponse("cart/","POST",params.accessToken, JSON.stringify(JSONData))).props;
...
export const removeProductFromCart = async (params): Promise<boolean> => {
    const { response } = (await getlambdaResponse("cart/removeProduct/"+decode(params.auth.accessToken).sub, "PUT", params.auth.accessToken, JSON.stringify(params.body))).props;
    if(response.error !== undefined) return false;
    return true;
}
...
export const insertProductInCart = async (params): Promise<boolean> => {
    const { response } = (await getlambdaResponse("cart/addProduct/"+decode(params.auth.accessToken).sub, "PUT", params.auth.accessToken, JSON.stringify(params.body))).props;
    if(response.error !== undefined) return false;
    return true;
}
