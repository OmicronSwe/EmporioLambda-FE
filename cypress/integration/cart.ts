describe('Test cart', () => {
    it('Check empty cart while signed out', () => { 
        ...
        cy.get("button").contains("Sign in").should(($button) => {
    ...
    it('Check cart remove single item while signed out', () => {
		...
		cy.get('a[href]').contains('Cart').click();
        cy.wait(4000).should(()=>{
            let lsContent : any = JSON.parse(localStorage.getItem('cart'));
            const toRemove = lsContent["items"][0]["id"];
            for(let i = 0; i <= lsContent.length; i=i+1)
            {
                if(lsContent["items"][i]["id"] === toRemove)
                {
                    lsContent.splice(i,1);
                }
            }
            cy.get('button[id='+toRemove+']').click();
            expect(localStorage.getItem('cart')).to.equal(JSON.stringify(lsContent))
        });
    })
