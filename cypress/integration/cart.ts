const checkIfLocalStorageContains = (array): boolean => {
  let result = false;
  array.items.forEach((elementlsContent) => {
    JSON.parse(localStorage.getItem("cart")).items.forEach((elementlsCart) => {
      if (
        elementlsContent.id === elementlsCart.id &&
        elementlsContent.quantity === elementlsCart.quantity
      ) {
        result = true;
      } else result = false;
    });
